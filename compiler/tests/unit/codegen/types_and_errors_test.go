...
func TestMoreErrorWrappers(t *testing.T) {
	...
	cases := []struct {
		...
		{"FunctionRequiresNamedArgsWithPos",
			codegen.WrapFunctionRequiresNamedArgsWithPos("foo", []string{"a", "b"}, pos), "foo"},
		{"FunctionRequiresNamedArgsWithPosNil",
			codegen.WrapFunctionRequiresNamedArgsWithPos("foo", []string{"a", "b"}, nil), "foo"},
