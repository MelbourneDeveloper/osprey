...
func TestDocumentationDeterministic(t *testing.T) {
	...
	if successfulRuns == 0 {
		t.Fatalf("❌ CRITICAL FAILURE: All %d documentation generation runs failed - this indicates a broken docs system", numRuns)
	}
...
func TestFunctionsIndexDeterministic(t *testing.T) {
	...
	if firstContent == "" {
		t.Fatalf("❌ CRITICAL FAILURE: No successful documentation runs found out of %d attempts - docs system completely broken", numRuns)
	}
