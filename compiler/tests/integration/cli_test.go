...
// TestCLIBasicFunctionality tests the main CLI commands
func TestCLIBasicFunctionality(t *testing.T) {
	// Skip if osprey binary doesn't exist
	if _, err := os.Stat("../../bin/osprey"); os.IsNotExist(err) {
		t.Skip("Osprey binary not found, skipping CLI tests")
	}
	...
	tests := []struct {
		...
		{
			name:     "docs_generation",
			args:     []string{"--docs"},
			wantExit: 0,
		},
		{
			name:     "hover_test",
			args:     []string{"--hover", "print"},
			wantExit: 0,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cmd := exec.Command("../../bin/osprey", tt.args...)
			err := cmd.Run()
			if err != nil {
				var exitErr *exec.ExitError
				if errors.As(err, &exitErr) {
					if exitErr.ExitCode() != tt.wantExit {
						t.Errorf("Expected exit code %d, got %d", tt.wantExit, exitErr.ExitCode())
					}
				} else {
...
// TestCLIWithFiles tests CLI with actual Osprey files
func TestCLIWithFiles(t *testing.T) {
	...
	testFile := "test_cli.osp"
	testContent := `print("Hello from CLI test")`
	...
	tests := []struct {
		...
		{
			name:     "show_ast",
			args:     []string{testFile, "--ast"},
			wantExit: 0,
		},
		{
			name:     "show_llvm",
			args:     []string{testFile, "--llvm"},
			wantExit: 0,
		},
		{
			name:     "compile_only",
			args:     []string{testFile, "--compile"},
			wantExit: 0,
		},
		{
			name:     "show_symbols",
			args:     []string{testFile, "--symbols"},
			wantExit: 0,
		},
	}
				...
				if errors.As(err, &exitErr) {
					if exitErr.ExitCode() != tt.wantExit {
						t.Errorf("Expected exit code %d, got %d. Output: %s",
							tt.wantExit, exitErr.ExitCode(), string(output))
					}
				} else {
					t.Errorf("Unexpected error: %v. Output: %s", err, string(output))
				}
			} else {
				// Command succeeded, check that we got some output
