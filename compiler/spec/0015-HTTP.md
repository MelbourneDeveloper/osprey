...
fn handleRequest(method: String, path: String, headers: String, body: String) -> String = 
    match method {
        "GET" => match path {
            ...
            "/api/stats" => generateStats()
            _ => "Not Found"
        }
        "POST" => match path {
            "/users" => createUser(body)
            "/api/data" => processData(body)
            _ => "Endpoint not found"
        }
        "PUT" => match path {
            "/users/1" => updateUser(body)
            _ => "Not Found"
        }
        "DELETE" => match path {
            "/users/1" => deleteUser()
...
// Functional programming approach to generate user list
...
fn createUserJson(id: Int) -> String = 
    "{\"id\": ${toString(id)}, \"name\": \"User${toString(id)}\", \"active\": true}"

fn appendUserJson(acc: String, user: String) -> String = 
    match acc {
        "[" => acc + user
        _ => acc + "," + user
    }

fn appendClosingBracket(userList: String) -> String = userList + "]"

fn createUser(body: String) -> String = 
    "{\"id\": 6, \"name\": \"New User\", \"message\": \"User created\"}"

fn updateUser(body: String) -> String = 
    "{\"id\": 1, \"name\": \"Updated User\", \"message\": \"User updated\"}"

fn deleteUser() -> String = 
    "{\"message\": \"User deleted\"}"

fn processData(body: String) -> String = 
    "{\"processed\": true, \"length\": ${toString(length(body))}}"

fn generateStats() -> String = 
    "{\"server\": \"osprey\", \"version\": \"1.0\", \"uptime\": \"${toString(getUptime())}\"}"

fn getUptime() -> Int = 3600  // Placeholder for actual uptime
```

This example demonstrates:

- **Raw HTTP handling** with direct method/path/headers/body access
- **Functional programming patterns** using map, fold, and pipe operators
- **Pattern matching** for request routing
- **JSON response generation** through functional composition
