...
typedef void (*ProcessEventHandler)(int64_t process_id, int event_type, char *data);
...
static void *process_monitor_thread(void *arg) {
    ...
    while (proc->is_running) {
        ...
        int max_fd = (proc->stdout_pipe[0] > proc->stderr_pipe[0]) ? 
                     proc->stdout_pipe[0] : proc->stderr_pipe[0];
        ...
        if (result > 0) {
            ...
            if (proc->handler) {
                ...
                snprintf(exit_code_str, sizeof(exit_code_str), "%lld", (long long)proc->exit_code);
