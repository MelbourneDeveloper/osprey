...
// Test event handler
static void test_event_handler(int64_t process_id, int event_type, char *data) {
    ...
    switch (event_type) {
        case 1: // PROCESS_STDOUT_DATA
            ...
            printf("TEST: Got stdout from process %lld: %s", (long long)process_id, data);
            break;
        case 2: // PROCESS_STDERR_DATA
            ...
            printf("TEST: Got stderr from process %lld: %s", (long long)process_id, data);
            break;
        case 3: // PROCESS_EXIT
            test_data.exit_count++;
            test_data.last_exit_code = atoll(data);
            printf("TEST: Process %lld exited with code: %lld\n", (long long)process_id, (long long)test_data.last_exit_code);
            break;
        default:
            printf("TEST: Unknown event type %d from process %lld\n", event_type, (long long)process_id);
            break;
    }
...
void test_basic_process_spawn() {
    ...
    int64_t process_id = spawn_process_with_handler("echo 'Hello from test process'", test_event_handler);
...
void test_multiple_processes() {
    ...
    int64_t pid1 = spawn_process_with_handler("echo 'Process 1'", test_event_handler);
    int64_t pid2 = spawn_process_with_handler("echo 'Process 2'", test_event_handler);
    int64_t pid3 = spawn_process_with_handler("echo 'Process 3'", test_event_handler);
    ...
    printf("Spawned processes: %lld, %lld, %lld\n", (long long)pid1, (long long)pid2, (long long)pid3);
...
void test_process_with_error() {
    ...
    int64_t process_id = spawn_process_with_handler("false", test_event_handler); // 'false' command returns exit code 1
...
void test_process_with_stderr() {
    ...
    int64_t process_id = spawn_process_with_handler("sh -c 'echo \"error message\" >&2'", test_event_handler);
...
void test_long_running_process() {
    ...
    int64_t process_id = spawn_process_with_handler("sh -c 'for i in 1 2 3; do echo \"Line $i\"; sleep 0.1; done'", test_event_handler);
...
void test_invalid_command() {
    ...
    int64_t process_id = spawn_process_with_handler("nonexistent_command_12345", test_event_handler);
