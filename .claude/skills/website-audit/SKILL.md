...
Identify the website source files in `website/`. The framework is 11ty (Eleventy) — templates are in `website/` with layouts, includes, and content files.
...
Note: robots.txt and sitemaps are often auto-generated by 11ty. If so, check the generator config rather than the output file.
...
- **Respect the framework** — edit 11ty templates/configs in `website/`, not generated output files in `website/_site`.
