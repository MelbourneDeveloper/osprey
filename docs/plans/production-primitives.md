...
|---|---|---|---|
| 1 | Lambdas at all (`fn(x) => x + 1`) | ✅ no-capture lambdas land everywhere; ✅ let-bound capturing lambdas work (inlined at call sites); ❌ **escaping closures** (a lambda *returned* as a value capturing its maker's state) still unsupported — lambda values are lifted without an env ([`crates/osprey-codegen/src/genfn.rs`](../../crates/osprey-codegen/src/genfn.rs)) | [`closures.md`](closures.md) |
...
- [ ] Land `closures.md` Phase 2 (escaping-closure capture); Phase 1 is done.
