...
| 3 | Error message payload threading through `Result<T, E>` | ✅ shipped: every builtin returns its real per-call message via the new `err_msg` slot; user-constructed `Error { message }` slots into any `Result<T, string>` shape; arithmetic chains propagate the inner error message per [ERR-PAYLOAD]; `TestErrorPayloadNoGenericFallback` keeps the deleted `"Error occurred"` global from sneaking back. Phase 3 (`Result<T, StringError>` typed payload) intentionally deferred — six-step migration plan preserved in [`error-payloads.md`](error-payloads.md#phase-3--discriminated-union-payload-types--deferred-decision-recorded). | [`error-payloads.md`](error-payloads.md) |
...
1. ~~`error-payloads.md`~~ — shipped end-to-end (Phases 1, 2, 4, 5, 6). Builtins emit per-call messages, user-constructed `Error { message }` round-trips through `Result<T, string>` for any T, and arithmetic chains preserve the inner error. Plan file retained because the Phase 3 (`Result<T, StringError>`) migration plan lives inside it; tick it off once Phase 3 lands.
...
- [x] Land all of `error-payloads.md`: Phases 1, 2, 4, 5, 6 shipped end-to-end with Go integration tests; Phase 3 (`Result<T, StringError>` typed payload) intentionally deferred with full migration plan recorded in the plan file. `tests/integration/error_payload_test.go` is the per-builtin tripwire and `TestErrorPayloadNoGenericFallback` blocks any regression to the legacy `"Error occurred"` global.
