...
List access returns `Result<T, IndexError>` for safety - no panics or exceptions.
...
- `handler`: Request handler function `(method: String, path: String, headers: String, body: String) -> String`
...
HTTP servers must call back into Osprey code to handle requests. The C runtime provides only the transport layer; all application logic and routing must be implemented in Osprey.
...
Osprey uses direct function pointer callbacks for performance.
...
All functions that can fail return a `Result` type. There are no exceptions, panics, or nulls.
...
#### `length(s: string) -> int`
**⚠️ IMPLEMENTATION ERROR**: The current code implementation is wrong. The `length` function should NOT return a `Result` type because string length operations cannot fail. String length is always computable and should return a plain `int`, not `Result<int, StringError>`.
...
Extracts a substring from start to end, returns Result for bounds safety.
...
String operations that can fail return Result types for explicit error handling.
...
Osprey is a functional language that uses functional patterns instead of imperative loops:
