...
> **Flavor layer — shared core (AST and above).**  Capability-based sandboxing and the effect system are one system across every flavor — entirely flavor-blind. The gated operations are ordinary calls that lower to `Expr::Call`; the permission gate enforces restrictions on the canonical `osprey_ast::Program` and in codegen, where restricted built-ins are simply never lowered or linked. Per [FLAVOR-BOUNDARY], no phase that decides what to block inspects which surface (`.osp` Default or `.ospml` ML) produced the program — security is enforced on the shared AST and below it in the runtime, never per-flavor. Both flavors are documented here: samples below appear in both flavors — Default (`.osp`) then its ML (`.ospml`) twin, each tagged with a flavor badge. The flag spellings shown are CLI concerns and are flavor-independent. See [Language Flavors](/spec/0023-languageflavors/) and [ML Flavor Syntax](/spec/0024-mlflavorsyntax/).
