...
> **Flavor layer — shared core (AST and above).**  HTTP is a runtime + canonical-AST concern and is flavor-blind. Every function here is an ordinary call lowering to `Expr::Call`, every `Result<T, string>` return is matched with `Expr::Match`, and the named record constructions (`HttpResponse { ... }`) lower to `Expr::TypeConstructor` — none of which carries surface identity. Both flavors are documented here: samples below appear in both surfaces — Default (`.osp`) then its ML (`.ospml`) twin, each tagged with a flavor badge. The Default surface uses named-argument calls like `httpGet(clientID: ..., path: ...)`; the ML surface spells the same calls with whitespace application and is described in [ML Flavor Syntax](/spec/0024-mlflavorsyntax/). Both surfaces meet at the same `osprey_ast::Program`, so the runtime never observes which flavor produced a request handler. See [Language Flavors](/spec/0023-languageflavors/).
...
The ML surface spells the identical calls with whitespace application: the named-argument Default calls become saturated tuple calls (`httpCreateClient (url, timeout)`, `httpGet (clientID, path, headers)`), and each `Success { value: x }` pattern becomes the payload-binding `Success x` over layout `match` arms:

```osprey-ml
match httpCreateClient ("http://api.example.com", 5000)
    Success clientID =>
        match httpGet (clientID, "/users", "")
