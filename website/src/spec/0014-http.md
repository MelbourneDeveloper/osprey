...
```osprey-ml
httpCreateServer : (int, string) -> Result<ServerID, string>

httpListen : (ServerID, (string, string, string, string) -> HttpResponse) -> Result<unit, string>

httpStopServer : ServerID -> Result<unit, string>
```
...
handleRequest (method, path, headers, body) =
    match method
        "GET" => match path
            "/health" => jsonResponse (200, "{\"status\":\"healthy\"}")
            "/users"  => jsonResponse (200, "{\"users\":[...]}")
            _         => jsonResponse (404, "{\"error\":\"not found\"}")
        "POST" => match path
            "/users" => jsonResponse (201, "{\"id\":3}")
            _        => jsonResponse (404, "{\"error\":\"not found\"}")
        _ => jsonResponse (405, "{\"error\":\"method not allowed\"}")
...
match httpCreateServer (8080, "127.0.0.1")
    Success serverID => httpListen (serverID, handleRequest)
    Error   message  => print "create failed: ${message}"
```
...
```osprey-ml
httpCreateClient : (string, int) -> Result<ClientID, string>

httpGet    : (ClientID, string, string) -> Result<HttpResponse, string>
httpPost   : (ClientID, string, string, string) -> Result<HttpResponse, string>
httpPut    : (ClientID, string, string, string) -> Result<HttpResponse, string>
httpDelete : (ClientID, string, string) -> Result<HttpResponse, string>
httpRequest : (ClientID, HttpMethod, string, string, string) -> Result<HttpResponse, string>

httpCloseClient : ClientID -> Result<unit, string>
```
...
match httpCreateClient (baseUrl: "http://api.example.com", timeout: 5000)
    Success clientID =>
        match httpGet (clientID: clientID, path: "/users", headers: "")
            Success response => print "status: ${response.status}"
            Error message => print "request failed: ${message}"
    Error message => print "client create failed: ${message}"
```